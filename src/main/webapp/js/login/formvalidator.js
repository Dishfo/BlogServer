...
function showLoginStatus() {
    ...
    console.log($.cookie('usernickname'));
	if ($.cookie('username')) {
		$('#loginBtn').hide();
		$('#registerBtn').hide();
		$('#persCenterBtn').show();
...
function sendRegisterInfo() {
	// 发送ajax请求
	$.ajax({
		url : '/user/registernow',
		async : false,// 同步，会阻塞操作
		type : 'POST',
		data : {
			user :JSON.stringify(registerInfo2Json())
...
function registerInfo2Json() {
	...
	newUserInfo.userPassword = $.md5($('#txt_userPwdRegister').val());
	newUserInfo.userEmail = $('#txt_userEmailRegister').val();
