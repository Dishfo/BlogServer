...
function initInputBox() {
	...
	if (article_id = $.getUrlParam('blogId')) {
	   	url = '/blog/blogdata/'+article_id;
	   	article_type = 'blog';
	} else if ((article_id = $.getUrlParam('codeId'))) {
		url = '/code/codedata/'+article_id ;
		article_type = 'code';
	}
...
function submitActInfo(type){
	...
	$.ajax({
		type : 'POST',
		async : false,
		url : (isBlogOrCode() == 'blog') ? '/blog/upload' : '/code/upload',
		data : {
...
function articleDetail() {
	...
	if (type == 'blog') {
		...
		newArticle.blogAuthor = user_info;
		...
		newArticle.codeAuthor = user_info;
