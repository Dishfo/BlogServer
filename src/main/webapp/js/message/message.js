...
$(function() {
	Comment.allocate({
		...
		id : 0,
		getCmtUrl : '/message/messagelist',
		setCmtUrl : '/message/addmessage'
	})
...
fn.showList = (function() {
	...
	function oneLi(_obj) {
		...
		for (var i = 0, len = _obj.response.length; i < len; i++) {
			...
			var t_name;
			if(t.messageUser!==undefined){
				t_name=t.messageUser.userNickName===undefined?t.messageUser.userName:t.messageUser.userNickName;
			}else {
				t_name=t.messageUsername;
			}
			t.messageContent = t.messageContent.replace(/\<\;/g, '<');
			t.messageContent = t.messageContent.replace(/\>\;/g, '>');
			str1 += '<li class="f-clear"><table><tbody><tr><td>'
					+ '<span class="username">' + t_name
		...
		console.log(_obj)
        var name;
		if(_obj.messageUser!==undefined){
            name=_obj.messageUser.userNickName===undefined?_obj.messageUser.userName:_obj.messageUser.userNickName;
		}else {
			name=_obj.messageUsername;
		}
...
fn.addCmt = function(_btn, _text, _parent) {
	...
	$.cookie("username")!=null){
		user_info.userName=$.cookie("username");
		email=$.cookie("useremail");

		if($.cookie("usernickname")!==undefined&&$.cookie("usernickname")!=null){
			user_info.userNickName=$.cookie("usernickname");
		}else {
            user_info.userNickName=$.cookie("username");
		}
	...
	new_message.messageUser = user_info;
		...
		success : function(_data) {
			...
			if (_data['result'] == 1) {
				...
				if (_parent == 0) {
                    ...
                    self.lists.push({
						'messageId' : id,
						'messageUsername' :user_info.userName,
						'messageDate' : time,
