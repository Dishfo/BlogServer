...
function loadDetail(data) {
	var codeBody = 
			...
			+ '</h2>'
			+ '	<span class="fh5co-post-date">' + new Date(data.codeDate).format('yyyy-MM-dd hh:mm:ss') + '</span>'
			+ ' <span class="fh5co-post-date">作者:' + data.codeAuthor.userName + '</span>'
			+ ' <span class="fh5co-post-date">浏览' + data.codeRead + '次</span>'
			+ ' <span class="fh5co-post-date">关键字：' + keywordsProcess(data.codeLabel) + '</span>';

	if (isSuperAdminOrSelf(data.codeAuthor.userName)) {
...
function findPreviousCode(codeId) {
	...
	$
			.ajax({
				type : 'POST',
				async : false,
				url : '/code/precode/'+$("#cid").val(),
                dataType : 'json',
				success : function(response) {
					if (response != 0) {
						previousCode = '<li class="previous"><a href="/code/codelib/'
							+ response +  '"><span class="glyphicon glyphicon-hand-left" aria-hidden="true"></span> 上一篇</a></li>';
					}else {
...
function findNextCode(codeId) {
	...
	$
			.ajax({
				type : 'POST',
				async : false,
				url : '/code/nextcode/'+$("#cid").val(),
				dataType : 'json',
				success : function(response) {
					if (response != 0) {
						nextCode = '<li class="next"><a href="/code/codelib/'
							+ response + '">下一篇 <span class="glyphicon glyphicon-hand-right" aria-hidden="true"></span></a></li>';
					}else {
