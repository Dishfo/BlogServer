...
function loadDetail(data) {
	var blogBody = 
			...
			+ '	</h2>' 
			+ '	<span class="fh5co-post-date">' + new Date(data.blogDate).format('yyyy-MM-dd hh:mm:ss') + '</span>'
			+ ' <span class="fh5co-post-date">作者:' + data.blogAuthor.userName + '</span>'
			+ ' <span class="fh5co-post-date">浏览' + data.blogRead + '次</span>'
			+ ' <span class="fh5co-post-date">关键字：' + keywordsProcess(data.blogLabel) + '</span>';

	if (isSuperAdminOrSelf(data.blogAuthor.userName)) {
...
function findPreviousBlog(blogId) {
	...
	$
			.ajax({
				type : 'GET',
				async : false,
				url : '/blog/preblog/'+$("#bid").val(),
				dataType : 'json',
				success : function(response) {
					if (response != 0) {
						previousBlog = '<li class="previous"><a href="/blog/blogdet/'
								+ response
...
function findNextBlog(blogId) {
	...
	$
			.ajax({
				type : 'GET',
				async : false,
				url : '/blog/nextblog/'+$("#bid").val(),
				dataType : 'json',
				success : function(response) {
					if (response != 0) {
						nextBlog = '<li class="next"><a href="/blog/blogdet/'
								+ response
